...
class Conv2d(Module):
    ...
    def __init__(
        self, image_size, in_channels, out_channels, kernel_size=(3, 3), stride=1
    ):
        super().__init__()
        ...
        kernels = [
            0.02
            * np.random.random((self.kernel_rows * self.kernel_cols, self.num_kernels))
            - 0.01
    ...
    def forward(self, images):
        ...
        for channel in range(self.in_channels):
            ...
            for row_start in range(
                0, layer_0.shape[2] - self.kernel_rows + 1, self.stride
            ):
                for col_start in range(
                    0, layer_0.shape[3] - self.kernel_cols + 1, self.stride
                ):
                    # print(row_start, col_start)
                    sect = self.get_image_section(
                        layer_0,
                        row_start,
            ...
            h_out = int(
                np.floor((self.image_rows - self.kernel_rows) / self.stride) + 1
            )
            w_out = int(
                np.floor((self.image_cols - self.kernel_cols) / self.stride) + 1
            )
            # print(h_out)
        ...
        result = reduce(lambda x, y: x + y, output) * Tensor(
            [1 / self.in_channels], autograd=True
        )
